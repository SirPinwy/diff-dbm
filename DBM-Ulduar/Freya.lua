...
function mod:SPELL_CAST_SUCCESS(args)
	...
	if args:IsSpellID(62678, 62873) then -- REVIEW! Summon Allies of Nature, never fired on . Instead there are Summon Wave spells: 62688 (confirmed), 62685 and 62686. To be confirmed adequacy via logs
	...
	elseif args.spellId == 62619 and self:GetUnitCreatureId(args.sourceName) == 33228 then -- Pheromones spell, cast by newly spawned Eonar's Gift second they spawn to allow melee to dps them while protector is up.
		DBM:AddMsg("Pheromones unhidden from combat log. Notify Zidras on Discord or GitHub") -- REVIEW! Pheromones never fired on . Instead there is only an emote event.
	...
	elseif spellId == 63601 then -- Strengthened Iron Roots
		DBM:AddMsg("Strengthened Iron Roots unhidden from combat log. Notify Zidras on Discord or GitHub") -- REVIEW! Strengthened Iron Roots never fired on . Instead there is only an emote event.
